classdef TrainingOptionsADAM < nnet.cnn.TrainingOptions
    % TrainingOptionsADAM   Training options for adaptive moment estimation (ADAM)
    %
    %   This class holds the training options for adaptive moment
    %   estimation (ADAM)
    %
    %   TrainingOptionsADAM properties:
    %       GradientDecayFactor         - Decay factor for moving average
    %                                     of gradients.
    ...
    %
    %   Example:
    %       Create a set of training options for training with ADAM. The
    ...
    methods(Access = public)
        function this = TrainingOptionsADAM(inputArguments)
            this = this@nnet.cnn.TrainingOptions(inputArguments);
            this.GradientDecayFactor = inputArguments.GradientDecayFactor;
        ...
        function out = saveobj(this)
            ...
            out.GradientDecayFactor = this.GradientDecayFactor;
    ...
    methods(Static)
        function inputArguments = parseInputArguments(varargin)
            try
                ...
                nnet.cnn.TrainingOptions.errorForInvalidOptions(parser.Unmatched,'adam');
        ...
        function this = loadobj(in)
            this = nnet.cnn.TrainingOptionsADAM(in);
        end
    end
end
...
defaultGradientDecayFactor = 0.9;
defaultSquaredGradientDecayFactor = 0.999;
defaultEpsilon = 1e-8;
defaultInitialLearnRate = 0.001;

p.addParameter('GradientDecayFactor', defaultGradientDecayFactor, @iAssertValidGradientDecayFactor);
...
inputArguments.GradientDecayFactor = results.GradientDecayFactor;
...
function iAssertValidGradientDecayFactor(x)
