classdef TextTableDataFactory < nnet.internal.cnn.ui.info.TableDataFactory
    % TextTableDataFactory   Factory for the TextTable that goes in the infostrip of the TrainingPlotView
    ...
    methods
        ...
        function [sectionName, sectionStructArr] = computeDuringTrainingCycleTableSection(~, epochInfo)
            ...
            % Num iterations per epoch
            sectionStructArr(2).RowID = 'NUM_ITERS_PER_EPOCH';
            sectionStructArr(2).LeftText = iMessageString('nnet_cnn:internal:cnn:ui:trainingplot:InfoStripNumItersPerEpochLabel');
            sectionStructArr(2).RightText = num2str(epochInfo.NumItersPerEpoch);
            % Max iterations
            sectionStructArr(3).RowID = 'MAX_ITERS';
            sectionStructArr(3).LeftText = iMessageString('nnet_cnn:internal:cnn:ui:trainingplot:InfoStripMaxItersLabel');
            sectionStructArr(3).RightText = num2str(epochInfo.NumIters);
        end
        ...
        function [sectionName, sectionStructArr] = computePostTrainingCycleTableSection(~, finalIteration, finalEpoch, epochInfo)
            ...
            sectionStructArr(2).RightText = iFormattedFinalIteration(finalIteration, epochInfo);
            % Num iterations per epoch
            sectionStructArr(3).RowID = 'NUM_ITERS_PER_EPOCH';
            sectionStructArr(3).LeftText = iMessageString('nnet_cnn:internal:cnn:ui:trainingplot:InfoStripNumItersPerEpochLabel');
            sectionStructArr(3).RightText = num2str(epochInfo.NumItersPerEpoch);
            % Max iterations
            sectionStructArr(4).RowID = 'MAX_ITERS';
            sectionStructArr(4).LeftText = iMessageString('nnet_cnn:internal:cnn:ui:trainingplot:InfoStripMaxItersLabel');
            sectionStructArr(4).RightText = num2str(epochInfo.NumIters);
        end
...
function str = iFormattedFinalIteration(finalIteration, epochInfo)
str = iMessageString('nnet_cnn:internal:cnn:ui:trainingplot:InfoStripFinalIterationValue', num2str(finalIteration), num2str(epochInfo.NumIters));
end
