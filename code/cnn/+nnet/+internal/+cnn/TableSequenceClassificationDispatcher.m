classdef TableSequenceClassificationDispatcher < nnet.internal.cnn.sequence.SequenceDispatcher
    ...
    properties (Access = ?nnet.internal.cnn.DistributableDispatcher)
        ...
        % - (seq2one) numObservations-by-1 categorical vector
    ...
    methods
        function this = TableSequenceClassificationDispatcher(dataTable, miniBatchSize, ...
            ...
            % networkResponseSize      - The number of classes to to be
            %                   classified, as inferred from the network.
            ...
            [this.ResponseSize, this.Categories] = iGetResponseSizeAndCategories( ...
                dispatcherFormat, this.Response, networkResponseSize);
            ...
            readStrategy = iGetReadStrategy( dispatcherFormat, this.ClassNames, networkResponseSize );
...
function Y = iReadSeq2SeqResponse( response, index, classNames, responseSize )
Ycell = iReadDatastore( response, index );
Ycategorical = Ycell{:};
Y = iDummify( Ycategorical, classNames, responseSize );
end
...
switch dispatcherFormat
    ...
    case 'seq2seq'
        fcn = @(response, index)iReadSeq2SeqResponse( response, index, classNames, responseSize );
end
end
...
if isempty( data )
    error(message('nnet_cnn:internal:cnn:TableSequenceClassificationDispatcher:XIsNotValidSequenceInput'));
end
end
...
function [responseSize, cats] = iGetResponseSizeAndCategories(dispatcherFormat, response, networkResponseSize)
% iGetResponseSizeAndCategories   Get the size of the response from the
% data, if the response is in-memory
switch dispatcherFormat
    case 'predict'
    ...
    case 'seq2one'
        ...
        responseSize = numel( cats );
    case 'seq2seq'
        responseSize = networkResponseSize;
        Ycell = iReadDatastore( response, 1 );
        Ycategorical = Ycell{:};
        cats = nnet.internal.cnn.util.categoriesFromResponse(Ycategorical);
end
end