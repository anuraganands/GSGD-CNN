function [dX, dW, dR, db] = lstmBackwardReturnLastM(X, W, R, b, c0, y0, Y, C, G, dZ) %#ok<INUSL>
% lstmBackwardReturnLastM   Propagate Long Short Term Memory layer backwards on the
% host
%   [dX, dW, dR, db] = lstmBackwardReturnLast(X, W, R, b, c0, y0, Y, C, G, dZ)
%   computes the backward propagation of the Long Short Term Memory layer.
%   Specifically, gradients are calculated relative to the forward
%   propagation function nnet.internal.cnngpu.lstmForwardReturnLast.
%   Arguments are defined as input data X, input weights W, recurrent
%   weights R, bias term b, initial cell state c0, initial hidden units y0,
%   output data Y, cell state C, layer gates G and output derivative dZ.
%   Definitions:
...
%   Y - Layer output              (H)x(N) array
%   C - Memory cell               (H)x(N)x(S) array
%   G - Layer gates               (4*H)X(N)x(S) array
%   dZ - Next layer derivative    (H)x(N) array
%
%   Outputs:
...
if S > 1
    ...
    tanhC = nnet.internal.cnngpu.tanhForward( C(:, :, S) );
    ...
    Ymm = nnet.internal.cnngpu.tanhForward( C(:, :, S - 1) ) .* G(oInd, :, S - 1);
    dR = iRecurrentWeightsDerivative(dR, dG, Ymm);
    ...
    for tt = (S-1):-1:2
        % Layer output derivative
        dY = iOutputDerivative(R(zInd, :), dG(zInd, :), ...
            R(iInd, :), dG(iInd, :), ...
            R(fInd, :), dG(fInd, :), ...
            R(oInd, :), dG(oInd, :));
        
        % Tanh activation of the cell state
        tanhC = nnet.internal.cnngpu.tanhForward( C(:, :, tt) );
        ...
        % Recurrent weights derivative
        Ymm = nnet.internal.cnngpu.tanhForward( C(:, :, tt - 1) ) .* G(oInd, :, tt - 1);
        dR = iRecurrentWeightsDerivative(dR, dG, Ymm);
    ...
    % Tanh activation of the cell state
    tanhC = nnet.internal.cnngpu.tanhForward( C(:, :, 1) );
...
% Derivatives
function dY = iOutputDerivative(Rz, dzpp, Ri, dipp, Rf, dfpp, Ro, dopp)
dY = Rz'*dzpp + Ri'*dipp + Rf'*dfpp + Ro'*dopp;
end
