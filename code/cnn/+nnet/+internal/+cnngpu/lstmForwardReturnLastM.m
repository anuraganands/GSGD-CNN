function [Y, C, G] = lstmForwardReturnLastM(X, W, R, b, c0, y0)
% lstmForwardReturnLastM   Propagate Long Short Term Memory layer forwards
...
for tt = 1:S
    ...
    % Layer output
    Y = nnet.internal.cnngpu.tanhForward( C(:, :, tt) ) .* G(oInd, :, tt);
end
...
% Nonlinear gate operations
G(zInd, :, tt) = nnet.internal.cnngpu.tanhForward( G(zInd, :, tt) );
G(ifoInd, :, tt) = nnet.internal.cnngpu.sigmoidForward( G(ifoInd, :, tt) );
end
