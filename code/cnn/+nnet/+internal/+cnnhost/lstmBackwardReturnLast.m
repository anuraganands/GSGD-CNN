function [dX, dW, dR, db] = lstmBackwardReturnLast(X, W, R, b, c0, y0, Y, C, G, dZ) %#ok<INUSL>
% lstmBackwardReturnLast   Propagate Long Short Term Memory layer backwards on the
% host
...
%   propagation function nnet.internal.cnnhost.lstmForwardReturnLast.
...
if S > 1
    ...
    Ymm = nnet.internal.cnnhost.tanhForward( C(:, :, S - 1) ) .* G(oInd, :, S - 1);
    ...
    for tt = (S-1):-1:2
        ...
        % Recurrent weights derivative
        Ymm = nnet.internal.cnnhost.tanhForward( C(:, :, tt - 1) ) .* G(oInd, :, tt - 1);
        dR = iRecurrentWeightsDerivative(dR, dG, Ymm);
